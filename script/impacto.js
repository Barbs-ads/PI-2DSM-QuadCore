async function carregarDados() {
  try {
    ...
    const percentualCartas = totalCartas > 0 ? Math.round((adotadas / totalCartas) * 100) : 0;

    
    document.getElementById("cartas-status").innerText = `${adotadas} de ${totalCartas}`;
    document.getElementById("cartas-percent").innerText = `${percentualCartas}% Concluído`;
    document.getElementById("cartas-restantes").innerText = `${restantes} crianças ainda aguardam!`;
    ...
    if (barraVoluntarios) {
      const percentualVoluntarios = dados.voluntariosTotal > 0 
